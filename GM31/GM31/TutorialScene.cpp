...
void TutorialScene::Fade_IN(uint64_t deltatime)
{
...
void TutorialScene::Fade_OUT(uint64_t deltatime)
{
	float time_D = static_cast<float>(deltatime) / 1000;
...
void TutorialScene::PlayerMove()
{
	...
	// 比率計算
	PL_rotation.y += diffrot * m_player.GetRATE_ROTATE_PLAYER();
	if (PL_rotation.y > PI)
	{
	...
	//重力の適用
	Object_Speed.y -= m_player.GetGRAVITY();
	...
	if (jumpflag)
	{
		Object_Speed.y += m_player.GetVALUE_JUMP_PLAYER();
	}
...
void TutorialScene::PlayerAdhesion()
{
...
void TutorialScene::AddSpeed(float initSpeed, Vector3 Speed)
{
	// 新しい速度を計算
...
void TutorialScene::SetSpeed(Vector3 Speed)
{
	Object_Position = Speed;
}

void TutorialScene::Collision_Hit()//弾とオブジェクトの当たり判定
...
void TutorialScene::RockonUpdate()
{
...
// デバッグフリーカメラ
void TutorialScene::debugFreeCamera()
{
...
void TutorialScene::PlayerMovetes()
{
	...
	if (jumpf) Object_Speed.y += m_player.GetVALUE_JUMP_PLAYER();
	...
	if (diff < -PI) diff += 2 * PI;
	rot.y += diff * m_player.GetRATE_ROTATE_PLAYER();
